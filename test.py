...
def copy_file(src_path, x=False):
    dst_path=os.path.join('dst', src_path)
...
def start_workers(command, queue):
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            future = executor.map(command, queue)
...
if __name__ == "__main__":
    ...
    srcs.remove('test.py')
    bfr = datetime.now()
    #start_workers(copy_file, srcs)
    for src in srcs:
        copy_file(src, True)
    print('Script finished after: ' + str(datetime.now() - bfr))
